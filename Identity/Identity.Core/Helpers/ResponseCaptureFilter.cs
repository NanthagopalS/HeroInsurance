...
public class ResponseCaptureFilter : IAsyncResultFilter
{
	...
	public ResponseCaptureFilter(ILogger<ResponseCaptureFilter> logger, IHttpContextAccessor httpContextAccessor,
		 IApplicationClaims applicationClaims, ILogRepository logRepository)
	{
		...
		_logRepository = logRepository ?? throw new ArgumentNullException(nameof(_logRepository));
	}
	...
	{
		var id = await _logRepository.InsertAPILogs(logsModel);
		return id;
	}

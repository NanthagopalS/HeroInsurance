   /*
   select * from dbo.[Insurance_GetVariantSuggestionForIFFCO] ('4CDB28C1-740C-4E3C-8877-7F137D1D43A8')    
   */
CREATE      function [dbo].[Insurance_GetVariantSuggestionForIFFCO]   
(      
@VariantId VARCHAR(100)
)      
returns @IFFCOVariantMapping TABLE(
	ICName VARCHAR(100),
	ScoreRank int,
	MScore float,
	VScore float,
...
@FuelType VARCHAR(10) = ''
	...
	INSERT INTO @IFFCOVariantMapping  
	SELECT 'ITGI' ICName,
			100 ScoreRank,
			100.00 MScore,
			100.00 MoScore,
	...
	IC.IsManuallyMapped
		FROM INSURANCE_VARIANT VARIANT WITH (NOLOCK)
		INNER JOIN MOTOR.ITGI_VehicleMaster IC WITH (NOLOCK) ON VARIANT.VariantId = IC.VariantId

...
BEGIN            
 BEGIN TRY            
   ...
   @IDVValue INT,@MinIdv INT,@MaxIdv INT,@RecommendedIdv INT, @YearId VARCHAR(50), @RTORegionCode VARCHAR(50), @VehicleVariant VARCHAR(50), @State_Id VARCHAR(50), @NoOfWheels VARCHAR(10), @NCBValueId VARCHAR(10),@ExShowRoomPrice VARCHAR(50),@ChassisPrice VARCHAR(50)  
 ...
 SELECT @RTOCode = rto.RTOCode,@RTORegionCode = BRCM.ModelRegionID, @CityName=ct.CityName,@Zone=BRCM.ModelZoneName,        
    @State_Id = BRCM.StateID FROM Insurance_RTO rto WITH(NOLOCK)           
   LEFT OUTER JOIN Insurance_City ct WITH(NOLOCK) ON ct.CityId=rto.CityId          
   LEFT OUTER JOIN MOTOR.Reliance_CV_RTOMaster BRCM WITH(NOLOCK) on BRCM.RTOId=rto.RTOId          
   WHERE (rto.RTOId = @RTOId OR RTOCode =LEFT(@VehicleNumber,4))          
          
   SET @PreviousInsurerCode = (SELECT InsuranceCompanyID as InsurerCode         
   FROM MOTOR.Reliance_CV_PreviousInsurerMaster WITH(NOLOCK) WHERE InsurerId = @PreviousInsurer)            
   ...
   @ExShowRoomPrice = VM.ExMumbaiBodyPrice,
   @ChassisPrice = VM.ExMumbai_Chasis_Price
   FROM [MOTOR].[Reliance_CV_VehicleMaster] VM WITH(NOLOCK)         
 ...
 SELECT @RTOCode = rto.RTOCode,@RTORegionCode = BRCM.ModelRegionID, @CityName=ct.CityName,@Zone=BRCM.ModelZoneName,        
  ...
  END       
   ...
   BEGIN        
    SET @FuelId = '2'        
   END        
   ELSE IF(@Fuel = 'CNG' OR @Fuel = 'PETROL+CNG')        
   BEGIN        
    SET @FuelId = '3'        
   END        
   ELSE IF(@Fuel = 'LPG' OR @Fuel = 'PETROL+LPG')        
   BEGIN        
    SET @FuelId = '4'        
   END        
   ELSE IF(@Fuel = 'BATTERY OPERATED' OR @Fuel = 'ELECTRIC')        
   BEGIN        
    SET @FuelId = '6'        
   END      
  ...
  where VariantId=@VariantId AND regNo=@VehicleNumber          
   ...
   @ChassisPrice ChassisPrice       
   IF(@VehicleTypeId = '88a807b3-90e4-484b-b5d2-65059a8e1a91')    
   BEGIN    
   SELECT TOP 1 'ProductCode' as ConfigName, PC.ProductCode as ConfigValue, CLD.UsageTypeId as UsageTypeId,CLD.UsageNatureId as SubUsageTypeId,
   CLD.PCVVehicleCategoryId as VehicleCategoryId
   FROM [MOTOR].[Reliance_CV_ProductCode] PC WITH(NOLOCK)     
   INNER JOIN Insurance_CommercialLeadDetail CLD WITH(NOLOCK) ON CLD.LeadId = @LeadId    
         WHERE PC.PolicyType = @PolicyTypeId AND CLD.VehicleCategoryId = PC.Category AND CLD.VehicleSubCategoryId = PC.SubCategoryId     
   AND CLD.CarrierTypeId = PC.CarrierType    
   END    
   ELSE    
   BEGIN    

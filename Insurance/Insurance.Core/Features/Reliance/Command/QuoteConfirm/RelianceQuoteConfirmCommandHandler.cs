...
namespace Insurance.Core.Features.Reliance.Command.QuoteConfirm
{
    public class RelianceQuoteConfirmCommand : QuoteConfirmRequestModel, IRequest<HeroResult<QuoteConfirmDetailsResponseModel>>
    {
    }
    public class RelianceQuoteConfirmCommandHandler :IRequestHandler<RelianceQuoteConfirmCommand, HeroResult<QuoteConfirmDetailsResponseModel>>
    {
        ...
        public RelianceQuoteConfirmCommandHandler(IQuoteRepository quoteRepository, IMapper mapper, IRelianceRepository relianceRepository, IApplicationClaims applicationClaims, IOptions<LogoConfig> options, IOptions<PolicyTypeConfig> policyTypeConfig)
        {
        ...
        public async Task<HeroResult<QuoteConfirmDetailsResponseModel>> Handle(RelianceQuoteConfirmCommand request, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                {
                    var result = await _relianceRepository.QuoteConfirmDetails(quoteResponseDB, quoteConfirmCommand, cancellationToken);
                    var requestBody = JsonConvert.DeserializeObject<PolicyDetails>(result.RequestBody); 
                    ...
                    if (oldResponse != null)
                    {
                        ...
                        result.quoteConfirmResponse.NewPremium = !string.IsNullOrEmpty(result.quoteConfirmResponse?.NewPremium) ? Convert.ToString(Math.Round(Convert.ToDecimal(result.quoteConfirmResponse?.NewPremium))) : Convert.ToString(Math.Round(Convert.ToDecimal(getOldResponse.TotalPremium)));
